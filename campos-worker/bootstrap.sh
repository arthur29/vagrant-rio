...
while ! ping -c1 192.168.12.2 &>/dev/null; do
  echo ping 192.168.12.2
done
while [ "echo $(echo quit | telnet 192.168.12.2 22 2>/dev/null | grep Connected)" = "" ] ; do
 sleep 10s 
done
while !  (scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no  -i /vagrant/ssh-key vagrant@192.168.12.2:/vagrant/cluster.token /vagrant/cluster.token &>/dev/null); do
 sleep 10s
done
